ACTION reporting::clearall(){
	...
	auto itr2 = _orders.begin();
	while(itr2 != _orders.end()){
    	itr2 = _orders.erase(itr2);
	}

	auto itr3 = _items.begin();
	while(itr3 != _items.end()){
    	itr3 = _items.erase(itr3);
	}

	auto itr4 = _voteassign.begin();
	while(itr4 != _voteassign.end()){
    	itr4 = _voteassign.erase(itr4);
	}

	auto itr5 = _notices.begin();
    while(itr5 != _notices.end()){
        itr5 = _notices.erase(itr5);
    }
}
